...
let rec executekrivine (prog: program) (env: environmentCLOS): answer = match prog with
  ...
  let rec lookup_closure (e, env) = match env with
  | [] -> raise Variable_not_intialized
  | (Var v, cl)::env' -> if v = e then cl else lookup_closure (e, env')
  | _ -> raise InvalidOperation

let test_krivine_machine (prog: program) (env: (exp * closure) list) =
  try
    let result = executekrivine prog env in
    match result with
    | I i -> print_endline ("Result: " ^ string_of_int i)
    | B b -> print_endline ("Result: " ^ string_of_bool b)
    | _ -> print_endline "Invalid result"
  with
  | Variable_not_intialized -> print_endline "Variable not initialized"
  | InvalidOperation -> print_endline "Invalid operation"
  | InvalidBigStepAnswerClosure -> print_endline "Invalid big step answer closure"
  | ReturnEmpty -> print_endline "Return empty"
